...
protected:
	...
	bool				Orphan;					// Sprites orphaned status
...
public:
	...
	PercentagePosition(false), UserData(NULL), Orphan(false), IgnoreCamera(false)
#if defined(IW_GAME_BOX2D)
	, Box2dBody(NULL)
#endif // IW_GAME_BOX2D_H_
	{
	...
	// Hit test
	virtual CIwGameActor* HitTest(float x, float y);
	...
	bool				CheckCollisionRect(CIwGameActor* other);
	...
	// Utility
	void				RemoveBody();
	CIwFVec2			PercPosToPos(float x, float y);
	float				getDistanceBetween(CIwGameActor* actor);
	float				getAngleBetween(CIwGameActor* actor);

	// Coordinate tranoform
	CIwFVec2			TransformPoint(float x, float y);	// Transform point by actors local angle / scale transform
...
public:
	CIwGameActorText() : CIwGameActor(), AutoHeight(false), FontLoaded(false), MarginIsPadding(false)			{ setActualClassType("actortext"); IsTappable = false; }
