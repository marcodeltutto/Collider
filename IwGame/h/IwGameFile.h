...
protected:
	// Properties
	CxFile				File;								// Marmalade file handle
...
public:
	CxFile				getFileHandle()						{ return File; }
...
public:
	...
	bool				Seek(int offset, CxFileSeekOrigin origin);
	void				Close();
...
protected:
	...
	CIwGameScript*			TargetScript;		// The file will be loaded into this script if it is set
...
public:
	...
	CIwGameDataFile() : IIwGameXomlResource(), File(NULL), State(State_Invalid), DataInput(NULL), TargetVariable(NULL), Converter(Converter_None), TargetScript(NULL) { setClassType("file"); }
	~CIwGameDataFile();
