...
public:
	...
	{
		unsigned int header_hash = CIwGameString::CalculateHash(header);
...
public:
	CIwGameHttpRequest() : ContentLength(0), Processed(false), Error(0), ContentAvailableCallback(NULL), ContentAvailableCallbackData(NULL), Post(false), AutoDelete(false) { PostData = new CIwGameHttpPostData(); }
	~CIwGameHttpRequest()
	{
