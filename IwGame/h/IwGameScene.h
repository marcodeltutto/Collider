...
protected:
	...
	IIwGameScriptEngine*		ScriptEngine;		// Mnaages scene scripts
	...
	CIwFMat2D					VirtualTransform;	// Virtual transform is used to scale, translate and rotate scene to fit different display sizes and orientations
	CIwFMat2D					Transform;			// Scene transform
	...
	CIwFVec2					PrevTouchVel;		// Previous touch velocity
...
public:
	...
	IIwGameScriptEngine*	getScriptEngine()						{ return ScriptEngine; }
	void					setScriptEngine(CIwGameString& type);
	...
	CIwFMat2D&				getVirtualTransform()					{ return VirtualTransform; }
	...
	void					fixActors(CIwGameActor* parent);
	...
	bool					getProperty(const char* element_name, CIwGameXomlProperty& prop);
	virtual bool			getProperty(unsigned int element_name, CIwGameXomlProperty& prop);
...
protected:
	...
	CIwFVec2				TransformedClipRect[2];		// Transformed clipping rect for the scene
...
public:
	CIwGameScene() :	IIwGameXomlResource(), Parent(NULL), Collidables(NULL), ResourceManager(NULL), NextFreeCollidable(0), Camera(NULL), MaxCollidables(0), MaxLayers(0), AllowSuspend(false), IsActive(true), 
						...
						ProgramManager(NULL), IsDragging(false), ScriptEngine(NULL), TickEnabled(false), PrevTouching(false)
#if defined(IW_GAME_BOX2D)
	...
	CIwFVec2		VirtualToScreen(float virtual_x, float virtual_y, bool ignore_translation = false);	// Returns a screen position frmo a virtual position
...
public:
	enum ActionType
	{
		...
		Action_ShowScene			= 4, 
		Action_KillScene			= 5, 
		Action_KillAllScenes		= 6, 
		Action_CallActions			= 7, 
		Action_SetAllSceneTimelines	= 8, 
		Action_AddModifier			= 9, 
		Action_ChangeModifier		= 10, 
		Action_CallScript			= 11, 
		Action_Inline				= 12, 
		Action_RemoteReq			= 13, 
		Action_Max					= 14, 
	};
