...
#define	IW_DEBUG_TRACE_NONE		0
#define	IW_DEBUG_TRACE_MIN		1
#define	IW_DEBUG_TRACE_MAX		2
extern int g_ShowTracing;
...
public:
	CIwGameSlotArray()
	{
		...
		Growth = 4;
		Size = 4;
		Elements = (T*)s3eMalloc(sizeof(T) * Size);
	...
	void grow(int growth)
	{
		if (growth <= 0)
			return;
		T* elements = (T*)s3eRealloc(Elements, sizeof(T) * (Size + growth));
		if (elements == NULL)
		{
			elements = (T*)s3eMalloc(sizeof(T) * Size);
			for (int t = 0; t < Size; t++)
	...
	void resize(int size)
	{
		...
		{
			T* elements = (T*)s3eRealloc(Elements, sizeof(T) * size);
			if (elements == NULL)
			{
				elements = (T*)s3eMalloc(sizeof(T) * size);
				for (int t = 0; t < size; t++)
