...
protected:
public:
	IIwGameXomlResource() : Managed(false), Parent(NULL) {}
...
//
//
// CIwGameXomlProperty - A basic XOML property
//
//
//
//
enum eCIwGameXomPropertyType
{
	PT_Invalid, 
	PT_String, 
	PT_Bool, 
...
struct CIwGameXomlProperty
{
	eCIwGameXomPropertyType Type;
	union
	{
		void*					p_data;
		bool					p_bool;
	...
	CIwGameXomlProperty() : Type(PT_Invalid) {}
};
...
public:
	...
	virtual void				setValue(const char* value, int count)	{ Value.setString(value, count); Changed = true; }
	...
	CIwGameXomlVariable() : Type(VT_String), Modified(false), Changed(true), Persist(false) {}
	virtual ~CIwGameXomlVariable() { Save(); }
...
struct CIwGameXomlVariableXML : CIwGameXomlVariable
{
	// Properties
protected:
	CIwGameXmlParser*	Parser;
	CIwGameXmlNode*		Root;					// Root node
public:
	CIwGameXmlNode*		getRoot()		{ return Root; }
	// Properties end
public:
	CIwGameXomlVariableXML() : CIwGameXomlVariable(), Root(NULL), Parser(NULL) { Type = VT_XML; }
...
public:
	...
	void					removeVariable(CIwGameXomlVariable* var);
...
public:
	CIwGameXomlResourceManager() : Parent(NULL) {}
	~CIwGameXomlResourceManager() { clearResources(); }

	bool					addResource(IIwGameXomlResource* resource);
	...
	static IIwGameXomlResource*	FindResource(unsigned int name_hash, unsigned int type_hash, IIwGameXomlResource* container = NULL);
	static IIwGameXomlResource*	FindResource(const char* name, unsigned int type_hash, IIwGameXomlResource* container = NULL);
	void					clearResources();
};
...
public:
	...
	static bool			CreateInstance(IIwGameXomlResource* parent, CIwGameScene* scene, CIwGameString* template_name, CIwGameString* parameters);
};
