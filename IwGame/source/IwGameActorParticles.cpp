...
//
//
CIwGameActorParticle::~CIwGameActorParticle()
{
...
void CIwGameActorParticle::Update(float dt, float dts)
{
	...
	if (Active)
	{
		...
		{
			if (!Attached && (LifeTime - dts) < SpawnDelay && Visual != NULL)
				Position = Parent->getVisual()->getTransform().TransformVec(Position);
...
void CIwGameActorParticles::RegenParticle(CIwGameActorParticle* particle)
{
	...
	{
		if (Visual != NULL)
			particle->Position = Visual->getTransform().TransformVec(particle->Position);
	}
}

bool CIwGameActorParticles::addParticle(CIwGameActorParticle* particle, CIwRect& src_rect, int count, float spawn_delay)
{
	...
	if (count != 0)
	{
		...
		{
			addParticle(particle->Clone(), src_rect, 0, delay);
	...
	for (int t = 0; t < Particles->getSize(); t++)
	{
		...
		if (particle != NULL)
		{
			...
			if (particle->Visible)
			{
				...
				CIwFVec2 pos = particle->Position;
				if (particle->Attached)
					pos = Visual->getTransform().TransformVec(pos);
			
				// Transfor actor properties to the visual
				visual->setPosAngScale(pos.x, pos.y, particle->Angle * 0.017453293f, particle->Scale.x, particle->Scale.y);
...
bool CIwGameActorParticles::LoadFromXoml(IIwGameXomlResource* parent, bool load_children, CIwGameXmlNode* node)
{
	...
	unsigned int vang_mode_hash = CIwGameString::CalculateHash("angvelmode");
	unsigned int vscale_mode_hash = CIwGameString::CalculateHash("scalevelmode");
	unsigned int vdepth_mode_hash = CIwGameString::CalculateHash("depthvelmode");
	...
	for (CIwGameXmlNode::_Iterator it2 = node->begin(); it2 != node->end(); ++it2)
	{
		...
		if (name_hash == CIwGameXomlNames::Particle_Hash)
		{
			...
			for (CIwGameXmlNode::_AttribIterator it = (*it2)->attribs_begin(); it != (*it2)->attribs_end(); ++it)
			{
				...
				if (attrib_hash == CIwGameXomlNames::Attached_Hash)
