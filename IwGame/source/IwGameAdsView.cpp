...
void CIwGameAdsView::Update(float dt)
{
	...
	{
		if (PrevTappedAd->AdSprite->HitTestNoClip((float)IW_GAME_INPUT->getTouch(0)->x, (float)IW_GAME_INPUT->getTouch(0)->y))
		{
		...
		if (data->Ad != NULL)
		{
			...
			else
			{
				...
				if (PrevTappedAd == NULL)
				{
					...
					{
						data->Tapped = true;
						if (data->AdSprite->HitTestNoClip((float)IW_GAME_INPUT->getTouch(0)->x, (float)IW_GAME_INPUT->getTouch(0)->y))
						{
							data->Colour.a = 0x7f;
							PrevTappedAd = data;
						}
...
void CIwGameAdsView::Draw(CIwGameScene* scene)
{
	if (!Visible)
		return;
	...
	for (int t = 0; t < NumAdsVisible; t++)
	{
		...
		if (data->Visible)
		{
			...
			data->AdSprite->setAngle(data->Angle * 0.017453293f);
