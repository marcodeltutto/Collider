...
/*		Font = IwGxFontCreateTTFont(Filename.c_str(), PointSize);
		...
		if (UsingFont != NULL)
		{
			if (UsingFont->getState() == State_Loaded)
			{
				Font = IwGxFontCreateTTFontFromBuffer((uint8*)UsingFont->getFontBuffer(), UsingFont->getFontBufferSize(), PointSize);
				State = State_Loaded;
				Delete = true;
				return true;
			}
...
//			File->setFileAvailableCallback(CIwGameFont_FileRetrievedAsyncCallback, this);
			...
			Font = IwGxFontCreateTTFontFromBuffer(FontBuffer, FontBufferSize, PointSize);
