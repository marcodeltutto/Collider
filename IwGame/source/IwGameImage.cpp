...
CIwTexture* CIwGameImage::getTexture()
{
	if (Load())
	{
		if (Image2D == NULL)
		...
		{
			if (Image2D != NULL)
				return Image2D->GetMaterial()->GetTexture(); 
		}
	}
...
void CIwGameImage::setFilter(bool enable)
{
	Filter = enable;
	if (Texture != NULL)
	...
	if (Image2D != NULL)
		Image2D->GetMaterial()->GetTexture()->SetFiltering(Filter);
	FilterSet = true;
}
...
bool CIwGameImage::Init(void* pixels, int width, int height, int pitch, CIwImage::Format format, bool modifiable)
{
	if (pixels == NULL || width <= 0 || height <= 0)
		return false;

//	uint8* texels = (uint8*)s3eMalloc(height * pitch);
//	memcpy(texels, pixels, height * pitch);
    ...
    Texture->CopyFromBuffer(width, height, format, pitch, (uint8*)pixels, NULL);
	Texture->Upload();
...
void CIwGameImage::SaveBmp(const char* filename)
{
	if (Image2D != NULL)
		Image2D->GetMaterial()->GetTexture()->GetImage().SaveBmp(filename);
	else
	if (Texture != NULL)
		Texture->GetImage().SaveBmp(filename);
}

void CIwGameImage::SaveTga(const char* filename)
{
	if (Image2D != NULL)
		Image2D->GetMaterial()->GetTexture()->GetImage().SaveTga(filename);
	else
	if (Texture != NULL)
		Texture->GetImage().SaveTga(filename);
}

void CIwGameImage::SavePng(const char* filename)
{
	if (Image2D != NULL)
		Image2D->GetMaterial()->GetTexture()->GetImage().SavePng(filename);
	else
	if (Texture != NULL)
		Texture->GetImage().SavePng(filename);
}

void CIwGameImage::SaveJpg(const char* filename, int quality)
{
	if (Image2D != NULL)
		Image2D->GetMaterial()->GetTexture()->GetImage().SaveJpg(filename, quality);
	else
	if (Texture != NULL)
		Texture->GetImage().SaveJpg(filename);
}
