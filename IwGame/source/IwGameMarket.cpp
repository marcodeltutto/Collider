...
void IIwGameMarket::Destroy()
{
	...
	switch (os)
	{
		case S3E_OS_ID_IPHONE:
			...
			break;
		case S3E_OS_ID_ANDROID:
			if (IW_GAME_MARKET_ANDROID != NULL)
			{
				IW_GAME_MARKET_ANDROID->Release();
				CIwGameMarketAndroid::Destroy();
			}
			break;
		case S3E_OS_ID_WINDOWS:
...
CIwGameError::LogError("transaction->m_TransactionStatus - ", CIwGameString((int)transaction->m_TransactionStatus).c_str());
    ...
    switch (transaction->m_TransactionStatus)
    {
        ...
        case S3E_PAYMENT_STATUS_RESTORED:
			...
			if (transaction->m_TransactionStatus == S3E_PAYMENT_STATUS_RESTORED)
...
CIwGameError::LogError("Restored");
            ...
            else
            {
				...
				CIwGameMarketProduct* product = IW_GAME_MARKET_IOS->findProduct(transaction->m_Request->m_ProductID);
				if (product != NULL)
				{
					CIwGameError::LogError("Product Purchased / Restored with ProductID - ", transaction->m_Request->m_ProductID);
					IW_GAME_MARKET_IOS->setPaymentTransaction(transaction);
					IW_GAME_MARKET_IOS->setTransactionReceipt(transaction->m_TransactionReceipt->m_ReceiptData, transaction->m_TransactionReceipt->m_ReceiptSize);
					IW_GAME_MARKET_IOS->NotifyReceiptAvailable();
//					s3eIOSAppStoreBillingCompleteTransaction(transaction, S3E_TRUE);	// Need to check to see if this needs to be called to release  the receipt
					IW_GAME_MARKET_IOS->setPurchased(product->ID);
...
bool CIwGameMarketiOS::StartPurchase()
{
	if (!Available)
		return false;
...
bool CIwGameMarketiOS::GetProductInfo()
{
	if (!Available)
		return false;
...
CIwGameError::LogError("**** - AsyncResponse - ", CIwGameString((int)result).c_str());
	...
	case S3E_ANDROIDMARKETBILLING_UNAVAILABLE:
		IW_GAME_MARKET_ANDROID->setStatus(MS_ERROR_UNAVAILABLE);
		IW_GAME_MARKET_ANDROID->NotifyError();
		break;
	}
