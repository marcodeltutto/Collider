...
{
	CIwGameXomlVariable* var = CIwGameXomlVariable::GetVariable(Parameter1, parent);
	if (var != NULL)
		return var->getValue();
...
{
	CIwGameXomlVariable* var = CIwGameXomlVariable::GetVariable(Parameter2, parent);
	if (var != NULL)
		return var->getValue();
...
{
	CIwGameXomlVariable* var = CIwGameXomlVariable::GetVariable(Parameter3, parent);
	if (var != NULL)
		return var->getValue();
...
{
	CIwGameXomlVariable* var = CIwGameXomlVariable::GetVariable(Parameter4, parent);
	if (var != NULL)
		return var->getValue();
...
{
#if defined(_DEBUG)
	if (findActions(actions->getNameHash()))
	{
		CIwGameError::LogError("Warning: Actions already exists, duplicate was ignored - ", actions->getName().c_str());
		delete actions;
		return;
	}
#endif	//(_DEBUG)
