...
void CIwGameAdsView::Update(float dt)
{
	...
	{
		if (PrevTappedAd->AdSprite->HitTestNoClip(IW_GAME_INPUT->getTouch(0)->x, IW_GAME_INPUT->getTouch(0)->y))
		{
		...
		if (data->Ad != NULL)
		{
			...
			else
			{
				...
				if (PrevTappedAd == NULL)
				{
					...
					{
						data->Tapped = true;
						if (data->AdSprite->HitTestNoClip(IW_GAME_INPUT->getTouch(0)->x, IW_GAME_INPUT->getTouch(0)->y))
						{
							data->Colour.a = 0x7f;
							PrevTappedAd = data;
						}
...
void CIwGameAdsView::Draw(CIwGameScene* scene)
{
	...
	for (int t = 0; t < NumAdsVisible; t++)
	{
		...
		if (data->Visible)
		{
			data->AdSprite->setPosition(data->Position.x * 8, data->Position.y * 8);
