...
{
	std::cout <<"!!! Entering CIwGameFacebook::PostWall\n";
	...
	std::cout <<"!!! CIwGameFacebook::PostWall is logged in\n";
...
//		s3eFBRequest_AddParamString(Request, "attachment", attachment.c_str());
        std::cout <<"!!! CIwGameFacebook::PostWall about to send request\n";
		s3eFBRequest_Send(Request, FBRequestCallback, NULL);
		/*
		if (s3eFBRequest_Send(Request, FBRequestCallback, NULL) == S3E_RESULT_ERROR)
		{
			std::cout <<"!!! CIwGameFacebook::PostWall request sent\n";
			s3eFBRequest_Delete(Request);
			return false;
		}
		else
