...
unsigned int CIwGameXomlNames::Transparent_Hash				= CIwGameString::CalculateHash("transparency");
...
void CIwGameXomlResourceManager::addResource(IIwGameXomlResource *resource)
{
#if defined(_DEBUG)
	if (findResource(resource->getNameHash(), resource->getClassTypeHash(), false))
	{
		CIwGameError::LogError("Warning: Resourcs already exists, duplicate was ignored - ", resource->getName().c_str());
		delete resource;
		return;
	}
#endif	//(_DEBUG)
...
//CIwGameError::LogError("CIwGameXoml::ProcessEnter: Memory used: ", CIwGameString(s3eMemoryGetInt(S3E_MEMORY_USED)).c_str());
	...
	for (CIwGameXmlNode::_Iterator it = node->begin(); it != node->end(); it++)
	{
		...
		for (_Iterator it2 = begin(); it2 != end(); ++it2)
		{
			...
			{
				found = true;
//CIwGameError::LogError("CIwGameXoml::Process: Creating instance: ", (*it)->GetName().c_str());
...
void CIwGameXoml::ApplyStyle(CIwGameStyle* style, CIwGameXmlNode* node)
{
	...
	{
		node->AddAttribute((*it)->Name, (*it)->Value);
	}
}
...
void CIwGameXomlVariableCondition::setValue(const char* value)
{
	...
	Changed = true;
	CIwGameScene* parent = Parent->getParent();
...
void CIwGameXomlVariableArray::setValue(const char* value)
{
	...
	for (int t = 0; t < Values.getSize(); t++)
	{
		...
		if (end > 0)
			Values.element_at(t)->setValue(values.getSubString(offset, end - offset).c_str());
		else
		if (values.getFindIndex() >= values.GetLength())
		{
			Values.element_at(t)->setValue(values.getSubString(offset, values.GetLength() - offset).c_str());
			break;
		}
...
void CIwGameXomlVariableManager::addVariable(CIwGameXomlVariable* variable)
{
	...
	{
		CIwGameError::LogError("Warning: Variable already exists, duplicate was ignored - ", variable->Name.c_str());
		delete variable;
		return;
	}
#endif	//(_DEBUG)
...
{
#if defined(_DEBUG)
	if (findVariable(CIwGameString::CalculateHash(name)))
	{
		CIwGameError::LogError("Warning: Variable already exists, duplicate was ignored - ", name);
		return NULL;
	}
#endif	//(_DEBUG)
...
bool CIwGameXomlAddVariable::LoadFromXoml(IIwGameXomlResource* parent, bool load_children, CIwGameXmlNode* node)
{
	...
	if (!name.IsEmpty())
	{
		...
		if (bind_xml != NULL && var != NULL && var->isArray())
