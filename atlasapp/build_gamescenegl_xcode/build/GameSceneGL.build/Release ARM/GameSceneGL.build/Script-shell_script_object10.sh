...
if [ "$CONFIGURATION" = "Debug" -o "$CONFIGURATION" = "Release" -o "$CONFIGURATION" = "Debug ARM" -o "$CONFIGURATION" = "Release ARM" -o "$CONFIGURATION" = "Debug X86_64 NaCL" -o "$CONFIGURATION" = "Release X86_64 NaCL" ]; then echo "AARCH64: skipping for x86 and arm and NaCl builds" && exit 0; fi
...
s3e_plink -o "/Users/deltutto/My_Collider/atlasapp/build_gamescenegl_xcode/build/${CONFIGURATION}${EFFECTIVE_PLATFORM_NAME}/GameSceneGL.s3e" "${TARGET_BUILD_DIR}/${EXECUTABLE_NAME}" --arch=armv8-a --compiler=gcc -u --usemap="/Users/deltutto/My_Collider/atlasapp/build_gamescenegl_xcode/build/${CONFIGURATION}${EFFECTIVE_PLATFORM_NAME}/GameSceneGL.map" --buildenv=XCODE_8.1 --config=aarch64 --config=release
